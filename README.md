...
This deploys the `expo start --web` target as a static site — not the native iOS/Android app, which isn't something you "deploy" the same way (see the web app's README for the distinction, or ask if you want a real installable build via EAS instead). `render.yaml` builds it with `npx expo export -p web`, which produces a static `dist/` folder (one `index.html`, one JS bundle — no client-side router here, so no rewrite rules are needed), and publishes that.
...
Verified locally before writing this: `npx expo export -p web` with `EXPO_PUBLIC_API_BASE_URL` set, served via a plain static file server, signed in and confirmed it loaded real data from the API — so the export mechanism itself is confirmed to work, though the actual Render static-site build has not been run (same caveat as everything else in this file: this environment can verify the mechanism, not Render's specific build environment).
