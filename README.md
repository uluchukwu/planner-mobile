...
## Deployment (Render) and PWA

This deploys the `expo start --web` target as an installable **Progressive Web App**, not the native iOS/Android app — that's a different thing (EAS Build), which isn't something you "deploy" to Render the same way; ask if you want that too. `render.yaml` builds it with `npx expo export -p web`, which produces a static `dist/` folder, and publishes that.

**What makes it a PWA, not just a static site:** `public/` holds everything Expo copies verbatim into the export — `manifest.json` (name, icons, `display: standalone` so it opens without browser chrome), `sw.js` (a service worker, registered from `index.ts` on web only), two generated icon PNGs (`icon-192.png`/`icon-512.png`, rendered from a checkmark-on-green-square SVG matching the web app's favicon — replacing the unedited `create-expo-app` placeholder icons, which had visible alignment-guide circles baked in and were never fit to ship), and a custom `index.html` (Expo reads this from `public/` instead of its own default template, since that's the only way to inject the `<link rel="manifest">` and Apple PWA meta tags — see `webTemplate.js` / `getFileFromLocalPublicFolder` in `@expo/cli` if you want the mechanism). `app.json`'s `web.themeColor`/`backgroundColor`/`description` feed the rest of the meta tags Expo generates automatically from config, and `web.favicon` was removed from `app.json` since it was injecting a second, conflicting `<link rel="icon">` pointing at that same placeholder image.

The service worker (`public/sw.js`) does exactly one job: stale-while-revalidate caching for same-origin static assets (the HTML, the JS bundle, the manifest), which is what lets the installed app *open* with no network at all. It deliberately does not touch API calls — those go to a different origin (the web app's URL), and the app already has its own read-only data cache (`src/cache.ts`) that knows which responses are safe to serve stale and shows a banner when it does; layering a second cache on top of that would just be two systems disagreeing about the same thing.

From the Render dashboard: **New → Blueprint**, connect this repo (`planner-mobile`). Unlike the web app's blueprint, one value can't be filled in automatically: `EXPO_PUBLIC_API_BASE_URL` needs the *web app's* deployed URL (e.g. `https://planner-xxxx.onrender.com`), which Render can't know ahead of time — set it manually in this service's Environment tab after the web app is deployed, then trigger a deploy (or redeploy) so the build picks it up. This has to be set *before* the build, not after: Expo inlines `EXPO_PUBLIC_*` vars at build time, so changing it later means triggering a fresh build, not just a restart.

**Verified locally, mechanism confirmed, Render's specific build environment not run** (same caveat as everything else in this file): exported with `EXPO_PUBLIC_API_BASE_URL` set, served via a plain static file server, and confirmed — the manifest resolves and is valid, the Apple PWA meta tag is present, the service worker reaches `activated` state, signing in loads real data from the API, and the shell assets are actually present in the service worker's cache afterward. Then, the load-bearing check: killed the static file server outright (not Playwright's offline-emulation flag, which turned out to interact unreliably with service-worker-controlled navigations in headless Chromium and isn't trustworthy for this) and reloaded — the login screen still rendered from the cached shell with the server genuinely unreachable. Zero console errors throughout.
...
**Investigated and found infeasible to build *and verify* in this environment — not merely deferred.** Before writing any code, the question was whether `expo-notifications` even works under `expo start --web` (the only target this environment can run — no Android SDK, `adb`, Java, or physical device). It doesn't, checked at runtime, not just by reading its source: `Notifications.scheduleNotificationAsync()` throws `"The method or property Notifications.scheduleNotificationAsync is not available on web"` — confirmed by installing the package, importing it in a throwaway probe screen, and running the actual call under Playwright (the probe and package have both since been removed; nothing from this shipped). `getPermissionsAsync()` does resolve on web, but that's the one piece already unusable without the scheduling half.

This matters for the same reason every other "not verified here" caveat in this README does — except one step further: everything else unverified-on-web (native SecureStore, native `AsyncStorage`) at least has a web-compatible fallback path that *is* tested. Notification scheduling has no web fallback at all, so shipping it would mean code that type-checks and nothing more — no test could distinguish it working from it silently no-op'ing, since it does the latter unconditionally on the one target available.

Two paths forward, both needing a real device or emulator (neither available here) before either could be verified:
- **Local scheduled notifications** — e.g. "remind me to check today's challenge at 9am" — client-only, no server involvement, gated on `notificationsEnabled` (already on `User`, already wired into mobile Settings). The natural first step, since it needs no new `/api/mobile/*` route.
- **Remote push** — a server-triggered notification — needs an Expo push token registered per device, a place to store it (`User` has no such column yet), and calling Expo's push API from the web app. Meaningfully larger, and blocked on the same device-testing gap as the local case.
